...
const RUN_AFTER_HOUR = Number.parseInt(process.env.LOYALTY_AWARD_RUN_HOUR || "13", 10);
const RUN_AFTER_MINUTE = Number.parseInt(process.env.LOYALTY_AWARD_RUN_MINUTE || "0", 10);
...
async function awardScheduledProfilePoints() {
  const localParts = getLocalDateParts();
  if (!shouldRunForLocalDate(localParts)) {
    return;
  }
  ...
  activeRun = (async () => {
    ...
    try {
      ...
      for (const customer of customersRes.rows) {
        ...
        const eventsToCheck = [];
        const birthdayDate = cleanText(customer.customer_birthday);
        const anniversaryDate = cleanText(customer.customer_anniversary);

        if (birthdayDate.slice(5, 10) === localParts.monthDay && config.birthdayPoints > 0) {
          eventsToCheck.push({
        ...
        if (anniversaryDate.slice(5, 10) === localParts.monthDay && config.anniversaryPoints > 0) {
          eventsToCheck.push({
      ...
      console.log(
        `[loyalty-scheduler] completed for ${localParts.localDate} in ${TIMEZONE}; awards=${totalAwards}`
      );
    } catch (error) {
      try {
