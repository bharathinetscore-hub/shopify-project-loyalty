...
async function awardScheduledProfilePoints({ force = false } = {}) {
  ...
  activeRun = (async () => {
    ...
    try {
      ...
      for (const customer of customersRes.rows) {
        ...
        for (const event of eventsToCheck) {
          ...
          pendingEmailNotifications.push({
            recipientEmail: cleanText(customer.customer_email) || null,
            customerName: cleanText(customer.customer_name) || "Customer",
            eventName: event.eventName,
      ...
      const { sendPointsEarnedEmail } = await import("./points-email.js");
      for (const notification of pendingEmailNotifications) {
        await sendPointsEarnedEmail(notification).catch((error) => {
          console.error("[loyalty-scheduler] earned email error:", error);
        });
      }
      if (!force) {
