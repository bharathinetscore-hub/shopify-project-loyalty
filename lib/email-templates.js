...
function textToHtml(text) {
  ...
  return `
    <div style="font-family: Arial, sans-serif; font-size: 16px; color: #1f2937; line-height: 1.6; white-space: pre-wrap;">
      ${safe.replace(/\n/g, "<br />")}
    </div>
  `.trim();
}
...
export async function getEmailTemplateMap() {
  ...
  for (const row of result.rows) {
    ...
    merged[key] = {
      ...
      isCustom: Boolean(cleanText(row.subject) || cleanText(row.text_body)),
    };
  }
...
export async function resolveEmailTemplate(templateKey, variables = {}) {
  ...
  return {
    ...
    html: textToHtml(renderTemplateString(selected.textBody, variables)),
  };
}
