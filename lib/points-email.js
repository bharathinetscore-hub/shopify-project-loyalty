...
async function getUserModeCounts() {
  ...
  return {
    loyaltyCount: Number(loyaltyRes.rows[0]?.total || 0),
    netsuiteCount: Number(netsuiteRes.rows[0]?.total || 0),
  };
}

export async function shouldSendLoyaltyPointsEmails() {
  const { loyaltyCount, netsuiteCount } = await getUserModeCounts();
  if (loyaltyCount > 0) return true;
  if (netsuiteCount > 0) return false;
  return true;
}
